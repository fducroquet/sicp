...
\begin{exe}[3.27]
    \begin{figure}
        ...
        \label{3.27fig}
    \end{figure}
    \autoref{3.27fig} shows some of the environments created during the 
...
\begin{exe}[3.36]
    ...
    \autoref{3.36fig} shows the environment structure in which the expression is 
    evaluated. The environments created when defining \vscm{a} and \vscm{b} are 
    similar to those created in \autoref{3.10}: an environment E1 (resp. E3) is 
    created for the evaluation of \vscm{(make-connector)}, then since 
    \vscm{(make-connector)} contains a \vscm{let}, a new environment E2 (resp. 
    E4) pointing to E1 (resp. E3) is created. The variables \vscm{value}, 
...
\begin{exe}[3.43]
    ...
    \autoref{3.43.1} shows how the account balances can be different than \$10, 
    \$20 and \$30 with the first version of the account-exchange program. The 
    \vscm{withdraw} and \vscm{deposit} procedures from each account are 
    ...
    \autoref{3.43.2} shows a case where the sum of the balances of the accounts 
    is not preserved after exchanges between \vscm{a1} and \vscm{a2} on one 
    side, \vscm{a2} and \vscm{a3} on the other side. Compared to the version 
    from \autoref{3.43.1}, the only transaction for a single account that does 
    not happen as if it were serialized is \vscm{((a2 'withdraw) 10)}: the first 
    exchange procedure sets the account’s balance to \$20 before the second 
    procedure can set it to \$10, and the final sum is \$50 instead of \$60.
\end{exe}
...
\begin{exe}[3.46]
    \autoref{3.46fig} shows how the implementation of \vscm{test-and-set!} can 
    fail if two procedures access the cell and find its contents at false before 
    both set its contents to true.
    \begin{figure}
        \centering
        \begin{tikzpicture}[>=Stealth]
            \matrix[time matrix] {
