...
\begin{exe}[2.29]
    ...
    \begin{enumerate}
        ...
        \item The total weight can be computed with:
            \scm{ch2/2.29b.scm}
        \item Since we defined a \vscm{branch-weight} procedure in b., 
            \vscm{balanced?} can be defined simply with:
            \scm{ch2/2.29c.scm}
        \item To convert to the new representation, the only things that need to 
            be changed are the \vscm{right-branch} and \vscm{branch-structure} 
            selectors:
            \scm{ch2/2.29d.scm}
    \end{enumerate}
\end{exe}
...
\begin{comp}
    ...
    \autoref{2.46} and to slightly modify the \vscm{segments->painter} to use 
...
\begin{exe}[2.49]
    ...
    \begin{enumerate}
        \item The painter drawing the outline can be defined as: 
            \scm{ch2/2.49a.scm}
        \item The painter drawing an “X” can be defined as: \scm{ch2/2.49b.scm}
        \item The painter drawing a diamond can be defined as: 
            \scm{ch2/2.49c.scm}
        \item The \vscm{wave} painter can be defined as: \scm{ch2/2.49d.scm}
    \end{enumerate}
\end{exe}
...
\begin{exe}[2.52]
    \ \vspace{-20pt}
    \begin{enumerate}
        \item Possible modified version of \vscm{wave}:
            \scm{ch2/2.52a.scm}
        \item Modified \vscm{corner-split}:
            \scm{ch2/2.52b.scm}
        \item Modified \vscm{square-limit}:
            \scm{ch2/2.52c.scm}
    \end{enumerate}
\end{exe}
...
\begin{exe}[2.64]
    ...
    \begin{enumerate}
        ...
        \item For \vscm{partial-tree}, the number of steps $T(n)$ as a function 
...
\begin{exe}[2.72]
    In the \vscm{encode-symbol} procedure given in \autoref{2.68}, only the 
...
\begin{exe}[2.73]
    ...
    \ \vspace{-20pt}
    \begin{enumerate}
        \item \vscm{same-variable?} and \vscm{number?} can’t be integrated to 
            the main dispatch because when they are true, \vscm{exp} has no 
            operator or operands.

	\item The procedures for derivation can be written in the following way, 
	    using \vscm{make-sum} and \vscm{make-product} from 
	    \secref[Representing-algebraic-expressions]{2.3.2}.
            \scm{ch2/2.73b.scm}

	\item To allow differentiation of exponents, it’s enough to call the 
	    following procedure, using \vscm{make-exporentiation} from 
	    \autoref{2.56}.

        \item The only change required is to exchange the arguments when calling 
            \vscm{put}, using e.g. \vscm{(put '+ 'deriv deriv-sum)} instead of 
            \vscm{(put 'deriv '+ deriv-sum)}.
    \end{enumerate}
\end{exe}
...
\begin{exe}[2.74]
    \ \vspace{-20pt}
    \begin{enumerate}
        \item The \vscm{get-record} procedure can be written using 
            ...
            \scm{ch2/2.74a.scm}
        \item \vscm{get-salary} can also be implemented with 
            ...
            \scm{ch2/2.74b.scm}
        \item The \vscm{find-employee-record} procedure can be written as:
            \scm{ch2/2.74c.scm}
        \item When Insatiable takes over a new company, the new company must tag 
            ...
            tag. It must install specific procedures for \vscm{get-record}, 
            \vscm{get-salary}, etc. into the operation-and-type table.
    \end{enumerate}
\end{exe}
...
\begin{exe}[2.81]
    ...
    \begin{enumerate}
        ...
        \item The \vscm{apply-coercion} procedure works correctly, however, it’s 
            unnecessary to attempt coercion to the same type so it should not be 
            attempted.
        \item The new version of \vscm{apply-generic} could be:
            \scm{ch2/2.81.scm}
    \end{enumerate}
\end{exe}
...
\begin{exe}[2.96]
    \ \vspace{-20pt}
    \begin{enumerate}
        \item The procedure \vscm{pseudoremainder-terms} can be defined as:
            \scm{ch2/2.96a1.scm}
            and the new version of \vscm{gcd-terms} is:
            \scm{ch2/2.96a2.scm}
	    On the example in \autoref{2.95}, \vscm{greatest-common-divisor} now 
...
\begin{exe}[2.97]
    \ \vspace{-20pt}
    \begin{enumerate}
        \item The \vscm{reduce-terms} and \vscm{reduce-poly} procedures can be 
            defined as:
            \scm{ch2/2.97a.scm}
        \item The only change needed besides adding the necessary entries to the 
            associative table is to redefine \vscm{make-rat} as:
            \scm{ch2/2.97b.scm}
