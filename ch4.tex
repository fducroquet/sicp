...
\begin{exe}[4.3]
    ...
    \vscm{variable?}, \vscm{self-evaluating?} and \vscm{application?}. If we had 
    kept Louis’ idea from the previous exercise to start procedure applications 
    with \vscm{call}, \vscm{application?} could have been assimilated into the 
    dispatch. Since we don’t keep this idea, we have to redefine the 
    \vscm{application?} predicate: if we keep it as \vscm{pair?} we can’t put it 
...
\begin{exe}[4.17]
    \begin{figure}
        ...
        \label{4.17fig}
    \end{figure}
    \autoref{4.17fig} shows the environment structures in which \vscm{<e3>} is 
...
\begin{exe}[4.20]
	...
	\item \vscm{Letrec} can be implemented as shown below. This is very 
	    similar to \autoref{4.16}, so the code could be mutualized:
	    \scm{ch4/4.20.scm}
	\item \autoref{4.20fig} shows the environment structures in which the 
...
\begin{exe}[4.22]
	...
	\autoref{4.16} by calling \vscm{scan-out-defines} in 
	\vscm{analyze-lambda}:
        \scm{ch4/4.22d.scm}
    \end{comp}
\end{exe}
...
\begin{exe}[4.32]
    ...
    following definition of \vscm{pairs} taken from \autoref{3.68}, where it 
...
\begin{exe}[4.71]
    ...
    \vscm{outranked-by} from \autoref{4.64}, with explicit delays the query
...
\begin{exe}[4.78]
    ...
    With the query \vscm{(last-pair ?x (3))} from \autoref{4.62}, the successive 
